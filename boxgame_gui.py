#!/usr/bin/python
#
# Also known as the dot game or pig pen.  Connect lines and try to be the player that completes a box.  If you 
# complete a box, you get another turn!  Made for fun, to learn Tkinter, bitwise operations, and hopefully to
# make a good AI.
#
# Caleb Braun
# 12/27/15
...
class Application(tk.Frame):
	...
	def get_game_mode(self):
		self.title_screen = tk.Canvas(self, width = self.WIDTH, height = self.HEIGHT, background='blue')
		self.title_screen.grid(rowspan=4)

		titlefont = tkFont.Font(family='Helvetica', size=48, weight='bold')
		buttonfont = tkFont.Font(family='Helvetica', size=36)
		title = tk.Label(self, text="THE DOT GAME", font = titlefont, bg='blue')
		...
		modeButton1 = tk.Button(self, text='vs. player', font=buttonfont, command=player_mode)
		modeButton1.grid(row=1)
		modeButton2 = tk.Button(self, text='vs. computer', font=buttonfont, command=player_mode)
		modeButton2.grid(row=2)

		# Quit button
		self.quitButton = tk.Button(self, text='Quito', command=self.quit)
	...
	def get_player_name(self):
		...
		self.title_screen.configure(bg='magenta')
